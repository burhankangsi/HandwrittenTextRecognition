...
class Window(QtGui.QMainWindow):
    ...
    def home(self):
        ...
        [self.loc_qlabel_signal.connect(x) for x in
         [lambda: loc_qlabel.setText(self.location_name), lambda: loc_qlabel.show()]]
        ...
        [self.img_crop_signal.connect(x) for x in
         [lambda: img_bound_qlabel.setText(",".join(str(v) for v in self.boundary_xy)),
    ...
    def extract_without_crop_message(self):
        extract_choice = QtGui.QMessageBox.question(self, "Continue?",
                                                    "Are you sure you want to continue without cropping the image?",
                                                    QtGui.QMessageBox.Yes | QtGui.QMessageBox.No)
    ...
    def load_image(self):
        ...
        qImg = QtGui.QImage(cv2.cvtColor(resized_img, cv2.cv.CV_BGR2RGB), res_width, res_height, res_width * 3,
